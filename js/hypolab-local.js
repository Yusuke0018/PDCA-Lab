        ...
        function loadTodayEvents() {
            const data = loadData();
            const todayKey = dateKeyLocal(new Date());
            const events = data.todayEvents && data.todayEvents[todayKey] ? data.todayEvents[todayKey] : [];
            ...
            if (events.length === 0) {
                listContainer.innerHTML = `
                    <div id="no-events-message" style="
                        text-align: center;
                        padding: 20px;
                        color: var(--text-secondary);
                        font-size: 14px;
                ...
                listContainer.innerHTML = events.map((event, index) => `
                    <div class="event-item" style="
                        ...
                        border-radius: 8px;
                        border: 1px solid var(--border);
                        ${event.completed ? 'opacity: 0.6;' : ''}
                    ">
                        <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
                            <input type="checkbox" 
                                   ${event.completed ? 'checked' : ''}
                                   onchange="toggleEventComplete(${index})"
                                   style="width: 20px; height: 20px; cursor: pointer;">
                            <div style="flex: 1;">
                                <div style="font-weight: bold; ${event.completed ? 'text-decoration: line-through;' : ''}">
                                    ${escapeHTML(event.title)}
                                </div>
                                ${event.time ? `<div style="font-size: 12px; color: var(--text-secondary);">🕰 ${escapeHTML(event.time)}</div>` : ''}
                                ${event.description ? `<div style="font-size: 12px; color: var(--text-secondary); margin-top: 4px;">${escapeHTML(event.description)}</div>` : ''}
                            </div>
                        </div>
                        <div style="display: flex; gap: 8px;">
                            <button onclick="editTodayEvent(${index})" style="
                                ...
                                padding: 4px;
                            ">✏️</button>
                            <button onclick="deleteTodayEvent(${index})" style="
                                background: transparent;
                                border: none;
                                color: var(--danger);
                                cursor: pointer;
        ...
        function showAddEventDialog(eventToEdit = null, editIndex = null) {
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 400px;">
                    ...
                    <form id="event-form" style="margin-top: 20px;">
                        <div class="form-group">
                            <label for="event-title">イベント名 <span style="color: var(--danger);">*</span></label>
                            <input type="text" id="event-title" required 
                                   value="${isEdit ? escapeHTML(eventToEdit.title) : ''}"
                                   placeholder="例: 会議、誕生日、ジム" 
                                   style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 8px;">
                        </div>
                        <div class="form-group" style="margin-top: 16px;">
                            <label for="event-time">時間</label>
                            <input type="time" id="event-time" 
                                   value="${isEdit && eventToEdit.time ? eventToEdit.time : ''}"
                                   style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 8px;">
                        </div>
                        <div class="form-group" style="margin-top: 16px;">
                            ...
                            <textarea id="event-description" 
                                      ...
                                      style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 8px; min-height: 60px; resize: vertical;">${isEdit && eventToEdit.description ? escapeHTML(eventToEdit.description) : ''}</textarea>
                        </div>
            ...
            document.getElementById('event-form').addEventListener('submit', function(e) {
                ...
                const eventData = {
                    ...
                    time: time,
                    description: description,
                    completed: isEdit ? eventToEdit.completed : false,
                    createdAt: isEdit ? eventToEdit.createdAt : new Date().toISOString()
                };
                
                saveTodayEvent(eventData, editIndex);
                modal.remove();
            });
        ...
        function saveTodayEvent(eventData, editIndex = null) {
            const data = loadData();
            const todayKey = dateKeyLocal(new Date());
            ...
            if (!data.todayEvents[todayKey]) {
                data.todayEvents[todayKey] = [];
            }
            
            if (editIndex !== null) {
                // 編集モード
                data.todayEvents[todayKey][editIndex] = eventData;
            } else {
                // 新規追加
                data.todayEvents[todayKey].push(eventData);
            }
            ...
            // 成功メッセージ
            showToast(editIndex !== null ? 'イベントを更新しました' : 'イベントを追加しました');
        }
        
        function editTodayEvent(index) {
            const data = loadData();
            const todayKey = dateKeyLocal(new Date());
            const events = data.todayEvents && data.todayEvents[todayKey] ? data.todayEvents[todayKey] : [];
            
            if (events[index]) {
                showAddEventDialog(events[index], index);
            }
        }
        
        function deleteTodayEvent(index) {
            ...
            if (data.todayEvents && data.todayEvents[todayKey]) {
                data.todayEvents[todayKey].splice(index, 1);
                saveData(data);
                loadTodayEvents();
        ...
        function toggleEventComplete(index) {
            const data = loadData();
            const todayKey = dateKeyLocal(new Date());
            
            if (data.todayEvents && data.todayEvents[todayKey] && data.todayEvents[todayKey][index]) {
                data.todayEvents[todayKey][index].completed = !data.todayEvents[todayKey][index].completed;
                saveData(data);
                loadTodayEvents();
                
                if (data.todayEvents[todayKey][index].completed) {
                    // 完了時にポイントを付与（オプション）
                    const pointsData = {
                        amount: 1,
                        reason: `イベント完了: ${data.todayEvents[todayKey][index].title}`,
                        timestamp: new Date().toISOString()
                    };
                    addPoints(pointsData.amount, pointsData.reason);
                    showToast('イベントを完了しました！ +1pt');
                }
            }
        }
        ...
        // 今日のイベント機能
        window.showAddEventDialog = showAddEventDialog;
        window.loadTodayEvents = loadTodayEvents;
        window.saveTodayEvent = saveTodayEvent;
        window.deleteTodayEvent = deleteTodayEvent;
        window.editTodayEvent = editTodayEvent;
